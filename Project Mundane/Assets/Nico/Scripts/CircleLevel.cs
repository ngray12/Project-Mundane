...
public class CircleLevel : MonoBehaviour
{
    ...
    float GetBiasedY()
    {
        ...
        if (rand < 0.45f)
        {
            return Random.Range(verticalRange * 0.3f, verticalRange); // Top
        }
        else if (rand < 0.9f)
        {
            return Random.Range(-verticalRange, -verticalRange * 0.3f); // Bottom
        }
        else
        {
            return Random.Range(-verticalRange * 0.3f, verticalRange * 0.3f); // Middle (rare)
        }
    }
